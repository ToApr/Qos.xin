...
namespace Qos.xin.Common
{
    public static class CalendarExtensions
    {
        ...
        private static MvcHtmlString GenerateHtml(string name, DateTime? date, string format)
        {
            if (date != null)
            {
                return MvcHtmlString.Create("<input type=\"text\" id=\"" + name + "\" name=\"" + name + "\" onfocus=\"WdatePicker({dateFmt:'" + format + "'})\" class=\"Wdate\" value=\"" + date.Value.ToString(format) + "\" />");
            }
            else
            {
                return MvcHtmlString.Create("<input type=\"text\" id=\"" + name + "\" name=\"" + name + "\" onfocus=\"WdatePicker({dateFmt:'" + format + "'})\" class=\"Wdate\" value=\"\" />");
            }
        }
    }
}
