    ...
    public static class InterfaceDesc
    {
        public static List<Inter> GetAllDesc()
        {
            ...
            for (int i = 0; i < types.Length; i++)
            {
                ...
                var ls=ns.Split('.');
                if (ls.Length > 2)
                    Url = ls.LastOrDefault() + "/";
                Interlist.Add(new Inter(types[i].BaseType == typeof(System.Web.UI.Page) ? types[i].Name + ".aspx" : Url+types[i].Name + ".ashx", ud._Desc, Parames));

            }//NPCWeb.Buss.Meeting   NPCWeb.Buss
